...
describe('staff access HTTP helpers', () => {
  it('requires an exact Origin on mutations', () => {
    assert.equal(hasSameOrigin(new Request('https://beautycorner.sa/api/admin/users', {
      method: 'POST', headers: { Origin: 'https://beautycorner.sa' },
    })), true);
    assert.equal(hasSameOrigin(new Request('https://beautycorner.sa/api/admin/users', {
      method: 'POST', headers: { Origin: 'https://evil.example' },
    })), false);
    assert.equal(hasSameOrigin(new Request('https://beautycorner.sa/api/admin/users', { method: 'POST' })), false);
  });
