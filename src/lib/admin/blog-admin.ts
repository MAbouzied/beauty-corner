...
export async function reserveAdminDraft(reservationId: string): Promise<AdminPost> {
  ...
  const authorName = 'فريق بيوتي كورنر';
...
export async function saveAdminPost(
  ...
  const authorName = input.author?.trim() || existing?.author || 'فريق بيوتي كورنر';
  const categoryId = adminCategoryDocumentId(categoryLabel);
  const authorId = adminAuthorDocumentId(authorName);
...
export function listAdminServices(): Array<{ id: string; title: string }> {
  return clinicServices.map((service) => ({ id: service.id, title: service.title }));
}
