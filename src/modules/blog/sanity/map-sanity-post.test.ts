...
describe('mapSanityPostToBlogPost', () => {
  ...
  it('rejects missing configuration fields with document context', () => {
    assert.throws(
      ...
      (error: unknown) => error instanceof SanityBlogPostMappingError && /post-1.*slug/i.test(error.message),
    );
  });
...
describe('mapSanityPosts', () => {
  ...
  it('skips malformed collection documents and reports their document IDs', () => {
    const issues: SanityBlogPostMappingError[] = [];
    const posts = mapSanityPosts(
      [baseDoc(), baseDoc({ _id: 'broken-post', body: [] })],
      imageConfig,
      (error) => issues.push(error),
    );

    assert.deepEqual(posts.map((post) => post.id), ['post-1']);
    assert.equal(issues.length, 1);
    assert.equal(issues[0]?.documentId, 'broken-post');
    assert.match(issues[0]?.message ?? '', /body is empty/i);
  });

  it('keeps direct document mapping strict', () => {
    assert.throws(
      () => mapSanityPostToBlogPost(baseDoc({ _id: 'broken-post', body: [] }), imageConfig),
      (error: unknown) => error instanceof SanityBlogPostMappingError && error.documentId === 'broken-post',
    );
  });
});
