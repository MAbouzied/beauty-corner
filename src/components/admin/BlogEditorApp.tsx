...
function EditorField({ post, onChange, onImage, onVideo, onReady, disabled }: { post: AdminPost; onChange: (state: EditorState, html: string) => void; onImage: () => void; onVideo: () => void; onReady: (editor: LexicalEditor) => void; disabled: boolean }) {
  const initialConfig = useMemo(() => ({ namespace: 'BeautyCornerBlog', theme: {}, nodes: [HeadingNode, QuoteNode, ListNode, ListItemNode, LinkNode, BlogImageNode, BlogVideoNode], onError(error: Error) { throw error; } }), []);
...
export default function BlogEditorApp({ post, services }: Props) {
  const [fields, setFields] = useState({ title: post.title, slug: post.slug, excerpt: post.excerpt, category: post.category || 'عام', author: post.author || 'فريق بيوتي كورنر', coverUrl: post.coverUrl, coverAlt: post.coverAlt, coverAssetId: post.coverAssetId, relatedServiceId: post.relatedServiceId });
