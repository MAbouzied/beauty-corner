...
function mutationError(error: unknown, fallback: string): Response {
  ...
  const duplicate = /مستخدم بالفعل|duplicate/i.test(message);
  const invalid = /مختصر غير صالح|Invalid blog slug/i.test(message);
...
export const POST: APIRoute = async (context) => {
  ...
  try {
    if (path[0] === 'assets') {
      ...
      if (!['image/jpeg', 'image/png', 'image/webp', 'image/gif'].includes(file.type.toLowerCase())) return json({ error: 'يسمح بصور JPG وPNG وWEBP وGIF فقط.' }, 400);
    ...
    return mutationError(error, 'تعذر حفظ المقال');
  }
};
...
export const PUT: APIRoute = async (context) => {
  ...
  try {
    ...
    return mutationError(error, 'تعذر تحديث المقال');
  }
};
